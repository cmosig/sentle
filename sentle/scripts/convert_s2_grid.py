...
df["epsg_code"] = df["Description"].apply(lambda desc: pd.read_html(StringIO(desc))[0][1].iloc[1])

df = df.rename(columns=dict(Name="name"))

print(df)

df[["name", "geometry", "epsg_code"]].to_file("../data/sentinel2_grid_stripped_with_epsg.gpkg")
