...
| `time_composite_method`          | `str`                       | `"mean"`                                     | How to aggregate the acquisitions within each `time_composite_freq` window: `"mean"` (default), `"median"`, `"min"` or `"max"`. NoData/masked pixels are ignored per band and pixel. Non-`"mean"` methods are currently supported for Sentinel-2 only (disable Sentinel-1). Only used when `time_composite_freq` is set.                                     |
