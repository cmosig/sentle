...
| `reuse_open_datasets`            | `bool`                      | `True`                                       | Keep each Sentinel-2 band raster open and reuse it across all subtiles of the same tile within a spatial chunk (instead of re-opening per subtile). Mainly speeds up CDSE: the first windowed read of a JP2 pays a one-time tile-structure discovery, so reuse amortizes it (≈2× faster on a 10 km area, more on larger ones). Set to `False` for lower memory. |
