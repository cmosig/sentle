...
| -------------------------------- | --------------------------- | -------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `provider`                       | `str`                       | `"planetary_computer"`                       | Which data catalog to download from: `"planetary_computer"` (default) or `"cdse"` (Copernicus Data Space Ecosystem). Both serve the same ESA L2A product, so reflectances are identical. CDSE is **Sentinel-2 only** (no Sentinel-1 RTC, no NBAR yet) and reads JP2s from CDSE S3, so it needs CDSE S3 credentials via the standard AWS chain (e.g. `AWS_PROFILE=cdse`, or `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY`). Currently slower per subtile than PC (JP2-over-S3). |
