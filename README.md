...
| `worker_timeout`                 | `float` or `None`           | `3600`                                       | Per-ptile timeout in seconds. A ptile that delivers no result within this budget aborts the run with a `multiprocessing.TimeoutError` instead of hanging forever behind a dead or wedged worker. This is a budget for a **single ptile**, not for the whole call — it is measured from the moment a ptile reaches the head of the retrieval queue, so it is unaffected by the total number of ptiles and by ptiles waiting their turn in the pool. (Time joblib spends pulling the next batch out of the job generator does count against whichever ptile is at the head, because both run on the same thread — a fraction of a second per batch.) Raise it for very wide `time_composite_freq` windows (one ptile then aggregates every acquisition in the window), or set it to `None` to disable. Only effective with `num_workers > 1`: at `num_workers=1` joblib runs the ptiles in the main process and cannot interrupt them (the HTTP and cloud-service timeouts below still apply). |
| `read_retries`                   | `int`                       | `2`                                          | How many extra attempts to make when reading a raster asset fails (403/404/503, a dropped connection, a stalled transfer). Once they are used up the **whole run is aborted** with a `SentleReadError` rather than skipping the band, so a cube that finishes always holds the same data as any other run over the same area. `0` fails on the first failure; there is no setting that restores the old skip-and-warn behaviour. |
...
- Every network wait is bounded, so a stalled server cannot hang a run indefinitely (see issue #87):
  - **Raster reads.** sentle enters a `rasterio.Env` that sets `GDAL_HTTP_CONNECTTIMEOUT=30`, `GDAL_HTTP_LOW_SPEED_LIMIT=1000` (bytes/s), `GDAL_HTTP_LOW_SPEED_TIME=30` (seconds below that rate before aborting) and `GDAL_HTTP_TCP_KEEPALIVE=YES` around every Sentinel-1/Sentinel-2 read. The low-speed knobs are used deliberately instead of `GDAL_HTTP_TIMEOUT`, which caps *total* transfer time and would abort legitimately slow-but-progressing cold range reads. Setting any of these as an environment variable overrides sentle's value, so the usual `GDAL_HTTP_*` tuning still works.
  - **STAC requests.** Catalog and search requests use a `(10, 60)` second (connect, read) timeout, retry 502/503/504 up to 15 times with capped backoff, retry a read timeout at most 3 times, and cap a server-sent `Retry-After` at 120 s.
  - **Asset signing.** Planetary Computer SAS tokens are fetched with a 120 s budget (`planetary_computer.sign` itself passes no timeout to `requests`).
  - **Cloud classification.** A worker waits at most `SENTLE_CLOUD_MASK_TIMEOUT` seconds (default `1800`, `0` disables) for the cloud-prediction service; if the service fails on a tile it now reports the error back instead of dying and leaving every worker blocked.
  - **Workers.** See `worker_timeout` above.
- A run either finishes with a complete cube or fails: a raster read that fails every attempt raises `SentleReadError` instead of being warned about and skipped, so two successful runs over the same area, time range and parameters produce the same data. Note that a *failed* run can leave a partially written store behind — treat it as garbage and rebuild rather than resuming.
