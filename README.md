...
- A Sentinel-2 subtile whose band downloads did not all succeed is skipped with a `subtile_dropped ... note=incomplete_band_set` warning rather than saved with a blank band. A single missing raw band already marks those pixels as NoData downstream, so no usable data is lost — but it does mean a flaky provider shows up as gaps plus warnings.
