...
| `S2_subtile_size` | `int` | `732` | Side length (in 10 m pixels) of the subtiles each Sentinel-2 tile (10980×10980) is split into for download. Must be a divisor of 10980, a multiple of 6 (so the 20 m/60 m bands read on an integer grid), and between 366 and 10980 — i.e. one of 366, 732, 1098, 1830, 2196, 3660, 5490, 10980. **Leave at 732 unless you know you need otherwise.** A smaller value only reduces the peak per-subtile working-array size (memory); it does *not* reduce download volume, because the Planetary Computer COGs are internally tiled at 512×512 and read a whole block at a time — so 366 actually transfers a bit more data and runs slightly slower than 732, and it changes the cloud classification slightly (smaller spatial context for the model). Works with cloud detection (the model input is padded to a multiple of 32 automatically). |
